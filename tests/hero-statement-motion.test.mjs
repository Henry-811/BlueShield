...
test('site pages load the current motion assets without stale browser cache',()=>{
 ...
 for(const page of pages){
  ...
  assert.match(html,/visual-2026\.css\?v=20260811-contact-email-1/,`${page} should version the motion stylesheet`);
  assert.match(html,/site\.js\?v=20260811-contact-email-1/,`${page} should version the motion script`);
 }
});
